...
def _(mo):
    file_browser = mo.ui.file_browser(
        initial_path='../', # Go up one since that is where all my data is. You might have to change that if you set up is different.
        multiple=True,
        filetypes=['.csv']
    ).form()
    file_browser
...
@app.cell(hide_code=True)
def _(datafiles, fit_results, mo, num_res, pd):
    _ = fit_results #for cell update only
